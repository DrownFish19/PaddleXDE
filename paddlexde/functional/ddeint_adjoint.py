...
class DdeintAdjointMethod(paddle.autograd.PyLayer):
    @staticmethod
    def forward(
        ctx,
        func,
        ...
        *adjoint_params,
        ...
        # 计算ans对于y_lags的梯度
        with paddle.no_grad():
            ans = ddeint(func, y0, t_span, lags, his, his_span, solver=solver)
    ...
    def backward(ctx, grad_y):
        ...
        with paddle.no_grad():
            ...
            # grad_params
            aug_state.extend([paddle.zeros_like(param) for param in adjoint_params])

            ##################################
            #    Set up backward DDE func    #
            ##################################
