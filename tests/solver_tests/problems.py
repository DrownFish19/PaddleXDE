...
class ConstantODE(paddle.nn.Layer):
    ...
    def __init__(self):
        ...
        self.a = paddle.create_parameter(shape=[1], dtype=paddle.float32, default_initializer=paddle.nn.initializer.Assign(paddle.to_tensor(0.2)))
        self.b = paddle.create_parameter(shape=[1], dtype=paddle.float32, default_initializer=paddle.nn.initializer.Assign(paddle.to_tensor(3.0)))
...
class SineODE(paddle.nn.Layer):
    ...
    def y_exact(self, t):
        return (-0.5 * t ** 4 * paddle.cos(2 * t) + 0.5 * t ** 3 * paddle.sin(2 * t) + 0.25 * t ** 2 * paddle.cos(2 * t) - t ** 3 + 2 * t ** 4 +
                (math.pi - 0.25) * t ** 2).unsqueeze(-1)
...
class LinearODE(paddle.nn.Layer):
    ...
    def __init__(self, dim=10):
        ...
        self.A = paddle.create_parameter(shape=A.shape, dtype=paddle.float32, default_initializer=paddle.nn.initializer.Assign(A))
    ...
    def y_exact(self, t):
        ...
        return paddle.stack([paddle.to_tensor(ans_) for ans_ in ans]).reshape([len(t_numpy), self.dim])
