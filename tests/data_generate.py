...
def get_data(num_timepoints=100):
    t = paddle.linspace(0., 4 * math.pi, num_timepoints)
    ...
    X = paddle.stack([paddle.concat([t.unsqueeze(0) for i in range(128)],axis=0), 
                      x_pos, 
                      y_pos
                      ], 
                     axis=-1) # [128,100,3]
...
plt.scatter(X_np[0, :, 1], X_np[0, :, 2], c=X_np[0, :, 0], cmap='coolwarm', edgecolors='black')
plt.xlabel('X')
plt.ylabel('Y')
