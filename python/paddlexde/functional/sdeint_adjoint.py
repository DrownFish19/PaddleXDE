...
class SdeintAdjointMethod(paddle.autograd.PyLayer):

    @staticmethod
    def forward(ctx, drift, diffusion, y0, t, rtol, atol, method, options, event_fn, adjoint_rtol, adjoint_atol, adjoint_method, adjoint_options,
        ...
        with paddle.no_grad():
            ans = sdeint(drift=drift, diffusion=diffusion, y0=y0, t=t, solver=method, rtol=rtol, atol=atol, options=options)
        ...
        with paddle.no_grad():
            ...
            def augmented_dynamics(t, y_aug):
                ...
                with paddle.set_grad_enabled(True):
                    ...
                    func_eval = drift(t if t_requires_grad else t_, y)
            ...
            for i in range(len(t) - 1, 0, -1):
                ...
                aug_state = sdeint(
                    drift=augmented_dynamics,
                    diffusion=augmented_diffusion,
                    y0=tuple(aug_state),
                    t=t[i - 1:i + 1],
                    solver=adjoint_method, rtol=adjoint_rtol, atol=adjoint_atol, options=adjoint_options, reverse=True)
...
def sdeint_adjoint(drift: callable, diffusion: callable, y0, t, *, rtol=1e-7, atol=1e-9, solver=None, options={'norm': _rms_norm}, event_fn=None,
    ...
    if adjoint_params is None and not isinstance(drift, nn.Layer):
    ...
    if adjoint_params is None:
        adjoint_params = tuple(find_parameters(drift)) + tuple(find_parameters(diffusion))
    else:
    ...
    solution = SdeintAdjointMethod.apply(drift, diffusion, y0, t, rtol, atol, solver, options, event_fn, adjoint_rtol, adjoint_atol,
                                         adjoint_solver, adjoint_options, not t.stop_gradient, *adjoint_params)
